...
def rewrite(content):
    """Return (new_content, wayback_count, direct_count)."""
    wb_count = [0]
    dir_count = [0]
    ...
    # Order matters: unwrap Wayback-wrapped ones first so the inner blog.centresource.com
    # reference isn't matched twice.
    new_content = WAYBACK_WRAPPED.sub(wb_sub, content)
    new_content = DIRECT.sub(dir_sub, new_content)

    return new_content, wb_count[0], dir_count[0]
...
def main():
    ...
    for filepath in files:
        ...
        new_content, wb, direct = rewrite(content)

        if wb == 0 and direct == 0:
            continue
        ...
        print(f"  {basename}: {direct} direct, {wb} wayback-wrapped")
