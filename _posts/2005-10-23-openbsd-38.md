...
Perhaps coolest of all comes some good news on the IPSEC front. Currently, OpenBSD's IPSEC support relies on the extremely clunky combination of [ipsecadm](<https://web.archive.org/web/20151127100413/http://www.openbsd.org/cgi-bin/man.cgi?query=ipsecadm>) for manual keying and [isakmpd.conf](<http://www.openbsd.org/cgi-bin/man.cgi?query=isakmpd>) for automatic keying, both of which are relatively obscure and daunting. OpenBSD 3.8 will introduce [ipsecctl](<http://www.openbsd.org/cgi-bin/man.cgi?query=ipsecctl&sektion=8>), which will obselete both isakmpd.conf and ipsecadm:
